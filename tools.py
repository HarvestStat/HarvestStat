...
def FDW_PD_CaliSeasonYear(stack, df, link_ratio, cs, cy):
    # Change Growing and Harvest season and year
    if len(cs) > 0:
        for s in cs:
            for m in cs[s]:
...
def FDW_PD_Compiling(df, shape_used):
    ...
    season_variables = ['season_name','growing_month','harvest_month']
...
def FDW_PD_RatioAdminLink(link, prod, over, mdx_pss):
    ...
    '''
    mdx_pss_name = ['product','season_name','growing_month','harvest_month','crop_production_system']
    # Validation
...
def FDW_PD_Sweeper(df,area_default='harvested'):
    ...
    # Minor changes -------------------------------------- #
    # Growing year and month (based on FDW's "season_date"; "growing_month" might be manually changed later)
    df['season_date'] = pd.to_datetime(pd.to_datetime(df['season_date']).dt.strftime('%Y-%m-01'))
    df['growing_year'] = df['season_date'].dt.year
    df['growing_month'] = df['season_date'].map(lambda x: '%02d-01' % x.month)
    df['year'] = df['growing_year'].copy()
    ...
    # Crop Calendar -------------------------------------- #
    print('- Crop calendar ------------------------------- #')
    temp = df[['product','season_name','growing_month','harvest_month']].drop_duplicates().sort_values('product').reset_index(drop=True)
    print(temp)
    print('')
    # ---------------------------------------------------- #
    ...
    cols_add = [
        ...
        'growing_year', 'growing_month', 'year', 'harvest_date', 'harvest_year', 'harvest_month',
